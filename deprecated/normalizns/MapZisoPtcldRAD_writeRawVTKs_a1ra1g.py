...
#b=os.getcwd()
for k in [0]:
    fskel = glob.glob(b+'\\'+'cellsRFP\*skel*')
    fVR=glob.glob(b+'\\'+'cellsRFP\*resample*')
    fVG = glob.glob(b+'\\'+'cellsGFP\*resample*') #GFP voxels #GFP voxels
    media=b.rsplit('\\',1)
    R=[];G=[];gN=[]
...
########## raw images labels are flipped, so resampled files are also flipped!!#################
        intenGFP=dataRFPV.GetPointData().GetScalars().GetTuple1
        intenRFP=dataGFPV.GetPointData().GetScalars().GetTuple1
