...
for media in dirlist:
    ...
    with open('%s_lagsUnscaled.pkl' % labs, 'rb') as inpt:
        (randNDY, randUDY, Norm, NormPermute, data, lineId) = pickle.load(inpt)
#    files = glob.glob(media+'\Norm*vtk')
    with open(
        os.path.join(
...
# =============================================================================
    for cell in data.keys():
        ...
        ACDY[labs][cell].append(
            autocorout(cell, Norm))
        ACU[labs][cell].append(
            autocorout(cell, randUDY))
        ACN[labs][cell].append(
